...
const setupL1 = async() => {
    ...
    l1Contract = new ethers.Contract(process.env.L1_CUSTOM_ADDRESS, erc20ABI, l1Signer)
}

//L1 Token Balance Check and if balance 0 -> give the token
const l1ERC20Balance = async() => {
    const l1balance = (await l1Contract.balanceOf(useAddr)).toString().slice(0,-8)
    ...
    await tx.wait()
    const newBalance = (await l1Contract.balanceOf(useAddr)).toString().slice(0,-8)
